...
export const useBadges = () => {
  ...
  const checkAndAwardBadges = useCallback((session: WorkoutSessionStats) => {
    ...
    const totalSquats =
      safeRead<number>(LS_TOTAL_SQUATS, 0) + (isSquat ? session.totalReps : 0);
    ...
    for (const badge of BADGES_CONFIG) {
      ...
      switch (badge.criteriaType) {
        case "workouts":
          isEarned = workoutCount >= badge.targetValue;
          break;
        case "reps":
          isEarned = totalSquats >= badge.targetValue;
          break;
        case "accuracy":
          isEarned = session.accuracy >= badge.targetValue;
          break;
        case "streak":
          isEarned = currentStreak >= badge.targetValue;
          break;
        case "time":
          isEarned = workoutHour >= badge.targetValue;
          break;
      }
